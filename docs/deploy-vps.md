...
# → พิมพ์ "ชื่อผู้ใช้ตอนล็อกอิน" + "รหัสผ่าน" ครั้งเดียว (เก็บแต่ hash ดูย้อนหลังไม่ได้)
# บัญชีนี้เป็น source=local — sync ผู้ใช้จาก ERP จะไม่แตะและไม่เขียนทับ คือกุญแจสำรอง
# ⚠️ บังคับมีก่อนเปิด ERP_USER_SYNC_ENABLED: ตั้งแต่ 5 ก.ย. 2569 ทุกคนจาก ERP ได้ role
#    เดียวกันหมดตาม ERP_USER_FIXED_ROLE (ค่าเริ่มต้น staff) → ERP สร้าง admin ให้ไม่ได้เลย
#    บัญชีนี้จึงเป็นทางเข้าระดับ admin ทางเดียวของระบบ ไม่ใช่แค่กุญแจสำรอง
```
