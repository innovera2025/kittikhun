...
- **สถานะปัจจุบัน (25 ส.ค. 2569): เปิดใช้แล้วในขอบเขตแคบ** ตามคำสั่งเจ้าของโปรเจค
  - ส่งได้เฉพาะ **ผลการนับของรอบที่ปิดแล้ว** เข้า `tbl_CountHdr` / `tbl_CountDtl`
  - ใช้ **บัญชีฐานข้อมูลคนละตัว** (`ERP_SQL_WRITE_USER`) จากบัญชีอ่านซึ่งยังเป็น
    `db_datareader` ล้วน — กฎเหล็กของทุกเส้นทางอ่านยังบังคับเหมือนเดิมทุกประการ
  - แตะได้แค่ 3 ตาราง (`tbl_CountHdr` · `tbl_CountDtl` · `RunningNumber`) บังคับด้วย
    `assertCountWriteSql()` · `DELETE` และ DDL ถูกห้ามทุกกรณี
  - ปิดอยู่โดยค่าเริ่มต้น ต้องตั้ง `ERP_WRITEBACK_ENABLED=true` อย่างจงใจ
  - หนึ่งรอบส่งได้ครั้งเดียว บังคับด้วย PK ของ `erp_writeback` — ⚠️ ฝั่ง ERP
    **ไม่มี unique** บน `VoucherNo`/`TransactionNo` จึงกันเอกสารซ้ำที่ปลายทางไม่ได้
  - **ไม่กระทบยอดคงเหลือของ ERP** — ตรวจแล้วไม่มี trigger บนทั้ง 3 ตาราง และยอด
    คงเหลือมาจาก `InventoryFlowHdr`/`Dtl` คนละเส้นทาง
