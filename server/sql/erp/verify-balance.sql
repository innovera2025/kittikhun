...
--
-- ทำไมต้องตรวจ: สูตรที่ฝ่าย ERP ส่งมา (สูตรที่ตัว ERP ใช้เอง) **ตรวจกับ `db_TCL` จริง
-- แล้วตรง 100% เมื่อ 22 ส.ค. 2569** — เทียบกับ `tbl_CountDtl.MainQty` สองรอบ:
-- CNT-2608-0001 93/93 · CNT-2608-0003 96/96 (ดู docs/erp-tcl-findings.md §6.6)
-- แต่ **ควรรันไฟล์นี้ซ้ำก่อน go-live เพราะข้อมูลเปลี่ยนได้** (แถวใหม่ที่ `IsClosed`/`Approved`
-- เป็น NULL, สินค้าที่ยังไม่มี movement, ยอดติดลบ ล้วนเกิดขึ้นใหม่ได้ตลอด)
--
