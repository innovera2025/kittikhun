...
ล็อกอิน **ถาม ERP สด ๆ ทุกครั้ง** ด้วย query ของลูกค้าเอง (`SELECT ... FROM menuuser WITH (NOLOCK)
LEFT OUTER JOIN Employee ... WHERE user_name = ?cUser`) แล้วเทียบ `a_Password` แบบ string ตรง ๆ
— ไม่มีการ hash เก็บรหัสผ่าน ERP ไว้ฝั่งเราอีกแล้ว และ **plaintext ของ ERP ไม่ถูกเก็บ/ไม่ลง log/
ไม่โผล่ในข้อความ error** ที่ใดเลย

> 🚨 **ล็อกอินต้องต่อ ERP ได้เท่านั้น** — ERP ล่มหรือเน็ตคลังหลุด = **ไม่มีใครล็อกอินใหม่ได้**
> ยกเว้นบัญชี break-glass (`source='local'` จาก `npm run create-admin`) · เครื่องที่ล็อกอินค้าง
> ไว้แล้วยังทำงานและ refresh token ต่อได้ตามปกติ · ลูกค้าปฏิเสธการแคช/ทำสำเนารหัสผ่านไว้ใช้
> ตอน ERP ล่มมาแล้วสองครั้ง ("ทุกอย่างจัดการที่อีอาร์พีหมด · ถ้าหาไม่เจอก็เข้าไม่ได้")

รอบ sync ผู้ใช้ (`user_credentials` แถว `source='erp'`/`legacy_pin`) **ยังอยู่แต่ไม่มีผลกับการล็อกอิน
แล้ว** — เป็นผู้สมัครถูกลบในคอมมิตแยก

| ความสามารถ | รายละเอียด |
|---|---|
| รหัสผ่าน | เทียบกับ `menuuser.a_Password` ที่ ERP แบบ string ตรง ๆ (constant-time) · argon2id + server pepper เหลือใช้กับบัญชี break-glass เท่านั้น (ระบบเราไม่ตั้ง/ไม่รีเซ็ตรหัสผ่านให้ใคร) |
